      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="flex flex-col sm:flex-row gap-3 md:gap-4 mb-4 md:mb-6">
          <button
            ...
            className={`flex-1 rounded-2xl md:rounded-3xl p-3 md:p-4 text-lg md:text-2xl font-bold transition ${
              activeTab === 'new'
          ...
          >
            <span className="block sm:inline">Pagamentos a Receber</span>
        ...
        {activeTab === 'payments' && (
          <div className="bg-white rounded-3xl p-6 shadow-xl border-4 border-yellow-300 mb-6">
            ...
            {clientsWithDebts.length === 0 ? (
              <div className="text-center py-12">
                <p className="text-2xl text-slate-500">Nenhum pagamento pendente</p>
              </div>
            ) : (
              <div className="space-y-4">
                {clientsWithDebts.map(client => (
                  <div
                    key={client.id}
                    className="bg-slate-50 rounded-2xl p-6 border-4 border-slate-200"
                  >
                    <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-4">
                      <div className="flex-1">
                        <h3 className="text-2xl md:text-3xl font-bold text-slate-800 mb-2">{client.name}</h3>
                        <p className="text-xl md:text-2xl font-bold text-red-600">
                          Total a Receber: R$ {client.totalDebtAmount.toFixed(2)}
                        </p>
                      ...
                      <button
                        ...
                        className="w-full sm:w-auto bg-green-500 hover:bg-green-600 text-white rounded-2xl px-6 md:px-8 py-3 md:py-4 flex items-center justify-center gap-2 md:gap-3 transition transform hover:scale-105"
                      >
                        <Check className="w-6 h-6 md:w-8 md:h-8" />
                        <span className="text-lg md:text-2xl font-bold">Dar Baixa</span>
                      </button>
                    </div>
                      ...
                      {client.debts.map(debt => (
                        ...
                        >
                          <div className="flex justify-between items-center">
                            <div>
                              <p className="text-lg font-semibold text-slate-800">{debt.description}</p>
        ...
        {activeTab === 'new' && (
          <>
        {/* Cliente */}
        <div className="bg-white rounded-3xl p-6 shadow-xl mb-6 border-4 border-yellow-300">
          ...
          <div className="flex gap-4">
            <input
              ...
              className="flex-1 text-2xl p-4 border-4 border-slate-300 rounded-2xl focus:border-yellow-500 focus:outline-none"
            />
            <button
              onClick={() => setShowClientModal(true)}
              className="bg-yellow-500 hover:bg-yellow-600 text-white rounded-2xl px-6 text-xl font-semibold transition"
            >
              Selecionar
            </button>
          </div>
        ...
        <div className="grid grid-cols-1 xl:grid-cols-3 gap-4 md:gap-6">
          ...
          <div className="xl:col-span-2">
            <div className="bg-white rounded-3xl p-6 shadow-xl mb-6 border-4 border-slate-200">
              <div className="flex flex-col sm:flex-row gap-3 md:gap-4 mb-4">
                <input
                  ...
                  className="flex-1 text-lg md:text-2xl p-3 md:p-4 border-4 border-slate-300 rounded-2xl focus:border-blue-500 focus:outline-none"
                />
                <button
                  onClick={() => setShowProductModal(true)}
                  className="bg-green-500 hover:bg-green-600 text-white rounded-2xl px-4 md:px-6 py-3 md:py-4 flex items-center justify-center gap-2 transition transform hover:scale-105"
                >
                  <Package className="w-5 h-5 md:w-6 md:h-6" />
                  <span className="text-base md:text-xl font-semibold">Novo Produto</span>
                </button>
              </div>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 md:gap-4 max-h-[50vh] md:max-h-[60vh] overflow-y-auto">
                {filteredProducts.map(product => (
                  <button
                    key={product.id}
                    onClick={() => addToCart(product)}
                    className="bg-slate-50 hover:bg-slate-100 rounded-2xl p-4 shadow-lg border-2 border-slate-200 transform transition hover:scale-105 active:scale-95 text-left"
                  >
          ...
          <div className="xl:col-span-1">
            <div className="bg-white rounded-2xl md:rounded-3xl p-4 md:p-6 shadow-xl border-4 border-yellow-300 sticky top-4">
              ...
              {cart.length === 0 ? (
                <p className="text-lg text-slate-500 text-center py-8">
                ...
                <>
                  <div className="space-y-3 mb-4 max-h-[40vh] overflow-y-auto">
                    {cart.map(item => (
                      <div
                        key={item.product.id}
                        className="bg-slate-50 rounded-2xl p-4 border-2 border-slate-200"
                      >
                  ...
                  <div className="border-t-4 border-slate-300 pt-4">
                    ...
                    <button
                      ...
                      className="w-full bg-yellow-500 hover:bg-yellow-600 disabled:bg-slate-300 disabled:cursor-not-allowed text-white rounded-2xl p-5 text-2xl font-bold transition transform hover:scale-105"
                    >
                      Salvar Fiado
                    </button>
                  </div>
            ...
            <div className="bg-white rounded-3xl p-8 max-w-md w-full shadow-2xl max-h-[90vh] overflow-y-auto">
              ...
              <div className="mb-6">
                <input
                  ...
                  className="w-full text-2xl p-4 border-4 border-slate-300 rounded-2xl focus:border-yellow-500 focus:outline-none mb-3"
                />
                <button
                  onClick={handleCreateClient}
                  disabled={!clientName.trim()}
                  className="w-full bg-blue-500 hover:bg-blue-600 disabled:bg-slate-300 disabled:cursor-not-allowed text-white rounded-2xl p-4 text-2xl font-semibold transition"
                >
                  + Criar Novo Cliente
                </button>
              </div>
                ...
                {clients
                  ...
                  .map(client => (
                    <button
                      ...
                      className="w-full bg-slate-100 hover:bg-slate-200 rounded-2xl p-5 text-left transition border-2 border-slate-300"
                    >
              ...
              <button
                ...
                className="w-full bg-slate-300 hover:bg-slate-400 text-slate-800 rounded-2xl p-4 text-2xl font-semibold transition"
              >
                Fechar
              </button>
            </div>
        ...
        {showProductModal && (
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
            <div className="bg-white rounded-3xl p-8 max-w-md w-full shadow-2xl">
              ...
              <div className="mb-6">
                <label className="block text-2xl font-semibold text-slate-700 mb-3">
                  Nome do Produto
                </label>
                <input
                  type="text"
                  ...
                  className="w-full text-2xl p-4 border-4 border-slate-300 rounded-2xl focus:border-green-500 focus:outline-none"
              ...
              <div className="flex gap-4">
                <button
                  ...
                  className="flex-1 bg-slate-300 hover:bg-slate-400 text-slate-800 rounded-2xl p-5 text-2xl font-semibold transition"
                >
                  Cancelar
                </button>
                <button
                  ...
                  className="flex-1 bg-green-500 hover:bg-green-600 disabled:bg-slate-300 disabled:cursor-not-allowed text-white rounded-2xl p-5 text-2xl font-semibold transition"
                >
                  Cadastrar
                </button>
              </div>
